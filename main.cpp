...
class password{
	...
	int verifyPassword(string user)
	{
	string x;
 ...
 {	bed b;
  ...
  fstream file("patient.txt");
   ...
   void adduser(){
	...
	cin>>pass;
	us=us+","+pass;
	// ofstream("password.txt");
	fstream newObj;	
	newObj.open("password.txt", ios::app);
	 newObj.seekp (0, ios::end);
	 newObj<<us<<endl;
	 newObj.close();
...
int main(){
	...
	cout<<endl;
	cout<<right<<setw(60)<<setfill(' ')<<"WELCOME TO IIITDMK HOSPITAL MANAGEMENT SYSTEM "<<endl;
	cout<<endl<<endl;
	cout<<right<<setw(60)<<setfill(' ')<<"------------------------------------------------"<<endl;
	
	cout<<right<<setw(40)<<setfill(' ')<<"1.Main Menu"<<endl;
	cout<<right<<setw(42)<<setfill(' ')<<"2.Create User"<<endl;
	cout<<right<<setw(35)<<setfill(' ')<<"3.Exit"<<endl;
